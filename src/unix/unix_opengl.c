...
#include <86box/unix_opengl.h>
#include <86box/unix_opengl_glslp.h>
...
extern void plat_resize(int w, int h, UNUSED(int monitor_index));
...
Uint32 opengl_sw_timer_set(Uint32 interval, void* param)
{
        sw_switch = 1;
...
{        
    ...
    if (!vid_mister && video_fullscreen)
    {
    	...
    	{
    		if (!sw_switch)
    		   sw_timer = SDL_AddTimer(50, opengl_sw_timer_set, NULL);
    		else
    		{	    	
    			sw_switch = 0;		    					  		    			
    ...
    if (resize_pending) {
        if (!video_fullscreen)
            opengl_resize(resize_w, resize_h);
        resize_pending = 0;
    } 
    ...
    ww = real_sdl_w;
    hh = real_sdl_h;   
    xx = video_fullscreen ? 0 : (w - ww) / 2;
    yy = video_fullscreen ? 0 : (h - hh) / 2;                
       
    if (!vid_mister && video_fullscreen && sw_x_scale >= 1 && sw_y_scale >= 1)
    {
    	ww = floor(0.5 + video_sw_req_w * sw_x_scale);
    	hh = floor(0.5 + video_sw_req_h * sw_y_scale);
    	xx = (real_sdl_w - ww) / 2;
    	yy = (real_sdl_h - hh) / 2;
